...
- Node.js on PATH (any recent Node with global `fetch`/`crypto.randomUUID` —
  v18+; this machine's system `node` at `/usr/bin/node` works, no need for
...
prompt <host> <project> <text> [--model NAME] [--timeout SECONDS]
    ...
    casing convention, not a fixed "Hostname Main" pattern — e.g. littlearch's
    project is titled `littlearch Main` (lowercase), not `Littlearch Main`.
...
approve <host> <threadId> <decision> [requestId]
    ...
    `remote_approve(host, threadId, decision)` — discovered by probing: a
    bare {threadId, decision} body 400s). If omitted, the script tries to
...
- **Client-generated IDs**: `threadId`/`commandId`/`messageId` are UUIDs
  generated locally — the dispatch response is only `{"sequence": N}`, no
  turn id comes back.
- **Completion contract**: polls `GET /api/orchestration/threads/:threadId`
  ...
  enumerated allow-list of terminal states — unrecognized values fall back
  to `"running"` server-side). It does **not** poll the message list (the
  assistant message only appears at completion). It also treats a
  **missing** `latestTurn` — which happens for the first instant after
  `thread.create`, before `thread.turn.start`'s effect lands — as still
  running, not as done; an earlier version of this script got this wrong
  and returned instantly with an empty message.
