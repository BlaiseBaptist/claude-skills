...
tool) instead of waiting for the user to capture one, which is useful for
verifying a visual change (e.g. waybar/sway config edits) before reporting it
done.
...
regardless of occlusion or which workspace the window is on. Nothing visible
happens: no workspace switch, no flicker.
