...
Tailscale's own HTTPS termination. Nothing is port-forwarded or publicly
exposed, and any device already on the tailnet can reach it.
...
2. Serve it locally on loopback, on a free port (pick something outside 3773 and
   ...
   Keep the PID; you'll kill it during cleanup.
...
remains. Leaving stray path mounts around after a test session is the same kind of mess
as leaving orphaned processes running. Clean up like you would any other probe.
